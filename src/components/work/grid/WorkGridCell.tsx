...
const WorkGridCell: FC<WorkGridCellProps> = ({ page }) => {
  ...
  return (
    <Link href={path} className="group">
      ...
      <div className="mt-1 text-lg font-medium sm:text-xl">
        <span className="text-accent">{title}</span>{" "}
        <span className="text-transparent transition-colors duration-100 group-hover:text-gray-500 dark:group-hover:text-gray-400">
          {subtitle}
        </span>
      </div>
    </Link>
